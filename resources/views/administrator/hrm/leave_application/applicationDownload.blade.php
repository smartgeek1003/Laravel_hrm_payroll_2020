...
==================================================-->
    <section id="payslip_new">
        <div class="container">
            ...
            <?php $applications= \App\LeaveApplication::all()->where('id',$id);?>
           @foreach($applications as $app)
            <div class="row">
                <div class="col-lg-12">
                        <table class="table">
                            ...
                            <tbody>
                                <tr>
                                    <th scope="row">{{__('Subject: Application for')}} {{\App\LeaveCategory::find($app->leave_category_id)->leave_category}} {{__('Leave')}}</th>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                       <table class="table">
                                <tr>
                                    <th scope="col">{{__('Sir,')}}</th>
                                </tr>
                                <tr>
                                    <td>{{__('This is to notify you I have to leave')}} <b>({{$app->start_date}} {{__('from')}} {{$app->end_date}} )</b> {{$app->reason}}</td>
                                </tr>
                                <tr><td></td></tr>
                                <tr><td>{{__('Therefore, I request you to kindly grant me leave for')}} ({{$app->start_date}} {{__('from')}} {{$app->end_date}} ). {{$app->during_leave}}</td></tr>
                ...
                <div class="col-lg-12 header_main clear-fix">
                    ...
                    <div class="brand_logo">
                        <b>{{__('Thanking you!')}}</b>
                     <p>{{__('Yours obediently,')}}</p>
                      <h5>{{\App\User::find($app->created_by)->name}}</h5>
                        <p>{{\App\User::find($app->created_by)->present_address}}</p>
                    </div>
            ...
            <div class="row">
             ...
             <div class="myprint">
                    ...
                    <a href="{{url('hrm/application_lists')}}"><button class="btn btn-primary"><i class="fa fa-download"></i> {{ __('Back') }}</button></a>
