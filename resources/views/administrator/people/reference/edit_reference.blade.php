...
@section('title', __('Edit reference'))
...
<div class="content-wrapper wow fadeInDown" data-wow-duration=".5s" data-wow-delay=".2s">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">{{ __('Edit reference') }}</li>
        </ol>
    </section>
    ...
    <section class="content">
        ...
        <div class="box box-default">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __('Edit reference') }}</h3>
            ...
            <form action="{{ url('people/references/update/'.$reference['id']) }}" method="post" name="reference_edit_form">
                ...
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            @else
                            <p class="text-yellow">{{ __('Enter reference type details. All field are required.') }} </p>
                            @endif
                        </div>
                        <!-- /.Notification Box -->

                        <div class="col-md-6">
                            <label for="name">{{ __('Reference Name') }} <span class="text-danger">*</span></label>
                            <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="name" id="name" class="form-control" value="{{ $reference['name'] }}" placeholder="{{ __('Enter name..') }}">
                                @if ($errors->has('name'))
                            ...
                            <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="email" id="email" class="form-control" value="{{ $reference['email'] }}" placeholder="{{ __('Enter email address..') }}">
                                @if ($errors->has('email'))
                                <span class="help-block">
                            ...
                            <div class="form-group{{ $errors->has('contact_no_one') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="contact_no_one" id="contact_no_one" class="form-control" value="{{ $reference['contact_no_one'] }}" placeholder="{{ __('Enter contact no..') }}">
                            ...
                            <div class="form-group{{ $errors->has('web') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="web" id="web" class="form-control" value="{{ $reference['web'] }}" placeholder="{{ __('Enter web..') }}">
                                @if ($errors->has('web'))
                                <span class="help-block">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group{{ $errors->has('present_address') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="present_address" id="present_address" class="form-control" value="{{ $reference['present_address'] }}" placeholder="{{ __('Enter address..') }}">
                            ...
                            <div class="form-group{{ $errors->has('contact_no_two') ? ' has-error' : '' }} has-feedback">
                                <input type="text" name="contact_no_two" id="contact_no_two" class="form-control" value="{{ $reference['contact_no_two'] }}" placeholder="{{ __('Enter address..') }}">
                ...
                <div class="box-footer">
                    <a href="{{ url('/people/references') }}" class="btn btn-danger btn-flat"><i class="icon fa fa-close"></i> {{ __('Cancel') }}</a>
                    <button type="submit" class="btn btn-primary btn-flat"><i class="icon fa fa-plus"></i> {{ __('Update reference') }}</button>
                </div>
            </form>
        </div>
        <!-- /.box -->
